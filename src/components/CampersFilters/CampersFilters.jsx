...
import { selectLocationFilter, selectFormFilter, selectEquipmentsFilters, changeFilters } from '../../redux/filterSlice'
import { selectFiltersMD5, setNewFiltersMD5, generateMD5 } from "../../redux/campersSlice"
import { campersForms } from "../../api/campers"
      ...
      <form onSubmit={handleFormSubmit}>
        <div className={css.location}>
          ...
          <div className={css.locationWraper}>
            <svg className={css.locationIcon}><use xlinkHref={`${icons}#icon-map`} /></svg>
            <input type="text" name="location" className={css.locationInput} value={locationFilter} id={locationId} onChange={handleLocationChanged} placeholder="City"/>
          </div>
        </div>
        <label>Filters</label>
        <div className={css.filter} >
          ...
          <ul>
            {Object.keys(equipmentsFilter).map((key) => { 
              ...
              return (
                <li key={myId}>
                  <input type="checkbox" name={key} className={css.fltrInput} checked={equipmentsFilter[key]} id={myId} onChange={handleEquipmentChecked}/>
                  <label htmlFor={myId} className={css.fltrLabel}>
                    <svg ><use xlinkHref={`${icons}#${equipmentsSVG[key]}`} /></svg>
                    {key}
                  </label>
                </li>
              )
        ...
        <div className={css.filter} >
          ...
          <ul>
            {campersForms.map((itm) => {
              ...
              return (
                <li key={myId}>                
                  <input type="radio" name="form" className={css.fltrInput} id={myId} value={itm} checked={itm == formFilter} onClick={handleFormChange} onChange={handleFormChange} />
                  <label htmlFor={myId} className={css.fltrLabel} >
                    <svg><use xlinkHref={`${icons}#${typeSVG[itm][1]}`} /></svg>
                    {typeSVG[itm][0]}
                  </label>
                </li>
              )
